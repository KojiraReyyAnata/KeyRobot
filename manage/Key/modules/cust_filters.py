...
from Key import dispatcher, LOGGER, DRAGONS
from Key.modules.disable import DisableAbleCommandHandler
from Key.modules.helper_funcs.handlers import MessageHandlerChecker
from Key.modules.helper_funcs.chat_status import user_admin
from Key.modules.helper_funcs.extraction import extract_text
from Key.modules.helper_funcs.filters import CustomFilters
from Key.modules.helper_funcs.misc import build_keyboard_parser
from Key.modules.helper_funcs.msg_types import get_filter_type
from Key.modules.helper_funcs.string_handling import (
    split_quotes,
    button_markdown_parser,
    escape_invalid_curly_brackets,
    markdown_to_html,
)
from Key.modules.sql import cust_filters_sql as sql
...
def list_handlers(update, context):
    ...
    if not all_handlers:
        send_message(
            ...
            f"Tidak ada filter yang disimpan di {chat_name}!",
        )
        return
...
def filters(update, context):
    ...
    if add is True:
        send_message(
            update.effective_message,
            f"Filter tersimpan '{keyword}' di *{chat_name}*!",
            parse_mode=telegram.ParseMode.MARKDOWN,
        )
