...
from Key import dispatcher, telethn
from Key.__main__ import (
    CHAT_SETTINGS,
    DATA_EXPORT,
    DATA_IMPORT,
    HELPABLE,
...
from Key.modules.helper_funcs.chat_status import dev_plus, sudo_plus
...
def load(update: Update, context: CallbackContext):
    ...
    try:
        imported_module = importlib.import_module(f"Key.modules.{text}")
    except:
...
def listmodules(update: Update, context: CallbackContext):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("Key.modules.", 1)[1]
