...
from Key import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        async for i in ubot2.iter_messages(
            "@logoKey", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
        fpath_ = glob.glob("./Key/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Key](https://t.me/Pusingeaak)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./Key/resources/fonts"):
            os.remove(font_)
