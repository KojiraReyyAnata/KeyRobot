...
from Key import TIGERS, WOLVES, dispatcher
from Key .modules.helper_funcs.chat_status import (
    bot_admin,
    is_user_admin,
    user_admin,
    user_admin_no_reply,
...
from Key.modules.sql import antiflood_sql as sql
...
def set_flood(update, context) -> str:
    ...
    if len(args) >= 1:
        ...
        if val in ["off", "no", "0"]:
            ...
            if conn:
                text = message.reply_text(f"Antiflood telah dinonaktifkan di {chat_name}.")
            else:
            ...
            if conn:
                text = message.reply_text(
                    f"Anti-flood telah diatur ke {amount} dalam obrolan: {chat_name}"
                )
            else:
                text = message.reply_text(
                    f"Berhasil memperbarui batas anti-flood ke {amount}!"
                )
...
def flood(update, context):
    ...
    if limit == 0:
        text = (
            msg.reply_text(
                f"Saya tidak menerapkan pengendalian flood di {chat_name}!"
            )
            if conn
