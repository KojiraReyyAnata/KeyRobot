...
from Key import aiohttpsession as aiosession
from Key.ex_plugins.dbfunctions import start_restart_stage
from Key.utils.http import get, post


async def restart(m: Message):
    if m:
        await start_restart_stage(m.chat.id, m.message_id)
    execvp(executable, [executable, "-m", "Key"])
