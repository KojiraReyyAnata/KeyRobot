...
import sys
import traceback
import Key.modules.sql.users_sql as sql
...
from Key.modules import ALL_MODULES
from Key.modules.helper_funcs.chat_status import is_user_admin
from Key.modules.helper_funcs.misc import paginate_modules
...
buttons = [
        ...
        InlineKeyboardButton(
            text="➕️ ᴛᴀᴍʙᴀʜᴋᴀɴ sᴀʏᴀ ᴋᴇ ɢʀᴏᴜᴘ ➕️", url="t.me/?startgroup=true"
        ),
    ],
    [
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module(f"Key.modules.{module_name}")
...
def help_button(update, context):
    ...
    try:
        if mod_match:
            ...
            text = f"Berikut adalah bantuan untuk *{HELPABLE[module].__mod_name__}* modul:\n{HELPABLE[module].__help__}"
...
def Key_about_callback(update, context):
    query = update.callback_query
    if query.data == "Key_":
        query.message.edit_text(
            text="🎧 Bantuan Perintah Music."
            "\npilih menu di bawah ini untuk melihat bantuan Music Key ",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="ᴘᴇʀɪɴᴛᴀʜ ᴀᴅᴍɪɴ", callback_data="Key_admin"),
                    InlineKeyboardButton(text="ᴘᴇʀɪɴᴛᴀʜ ʙᴏᴛ", callback_data="Key_notes"),
                 ],
                 [
                    InlineKeyboardButton(text="ᴘᴇʀɪɴᴛᴀʜ ᴘʟᴀʏ", callback_data="source_"),
                    InlineKeyboardButton(text="ᴘᴇʀɪɴᴛᴀʜ ᴇxsᴛʀᴀ", callback_data="Key_credit"),
                 ],
                 [
                    InlineKeyboardButton(text="ᴋᴇᴍʙᴀʟɪ", callback_data="Key_support"),
                 ]
                ]
            ),
        )
    elif query.data == "Key_back":
        first_name = update.effective_user.first_name
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
                PM_START_TEXT.format(
        ...
        query.message.edit_text(
            ...
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(text="ᴋᴇᴍʙᴀʟɪ", callback_data="Key_")]]
            ),
        )
    elif query.data == "Key_notes":
        query.message.edit_text(
            text="🤖 PERINTAH BOT"
        ...
        query.message.edit_text(
            text="*📝 Bantuan Perintah Key*"
            "\npilih tombol dibawah untuk melihat bantuan perintah Key.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="🎧ᴍᴜsɪᴄ", callback_data="Key_"),
                    InlineKeyboardButton(text="🤖ᴍᴀɴᴀɢᴇ", callback_data="help_back"),
                 ],
                 [
                    InlineKeyboardButton(text="ᴋᴇᴍʙᴀʟɪ", callback_data="Key_back"),
        ...
        query.message.edit_text(
            text="👨‍💻 Untuk yang ingin berdonasi sebagai ucapan terimakasih kepada Pembuat Key,"
            "\n\n • Silahkan Hubungi Owner atau klik dibawah ini",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="**ᴋᴇʏ**", url="t.me/Pusingeaak"),
                 ],
                 [
                    InlineKeyboardButton(text="ᴋᴇᴍʙᴀʟɪ", callback_data="Key_back"),
                 ]
...
def send_settings(chat_id, user_id, user=False):
    if user:
        ...
        dispatcher.bot.send_message(
            user_id,
            text=f"Modul mana yang ingin Anda periksa {chat_name} pengaturan untuk?",
            reply_markup=InlineKeyboardMarkup(
...
def settings_button(update: Update, context: CallbackContext):
    ...
    try:
        if mod_match:
            ...
            text = f"*{escape_markdown(chat.title)}* memiliki pengaturan berikut untuk *{CHAT_SETTINGS[module].__mod_name__}* module:\n\n{CHAT_SETTINGS[module].__chat_settings__(chat_id, user.id)}"
            ...
            chat = bot.get_chat(chat_id)
            query.message.reply_text(
                f"Hai, yang di sana! Ada beberapa pengaturan untuk {chat.title} - pergi ke depan dan memilih apa kamu tertarik.",
                reply_markup=InlineKeyboardMarkup(
                    paginate_modules(
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(
        Key_about_callback, pattern=r"Key_", run_async=True
    )
