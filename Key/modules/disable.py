...
from Key import dispatcher
from Key.modules.helper_funcs.handlers import CMD_STARTERS, SpamChecker
from Key.modules.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from Key.modules.sql import disable_sql as sql
    ...
    def disable_module(update: Update, context: CallbackContext):
        args = context.args
        if len(args) >= 1:
            disable_module = "Key.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        args = context.args
        if len(args) >= 1:
            enable_module = "Key.modules." + args[0].rsplit(".", 1)[0]
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        return f"Perintah berikut saat ini dibatasi:\n{result}"
